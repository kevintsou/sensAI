...
| ~~語法檢查~~ | **已改為做。** 原本的理由是「C 交給 clangd」，但那假設了目標機器上裝有 clangd 或編譯器 —— 韌體開發機不一定有。改由模型回報，rule_id 固定為 `syntax-error`。不外掛 `as` / `gcc`：那需要 include 路徑與交叉編譯 toolchain，等於重做一次 clangd |
