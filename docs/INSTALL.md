...
| 層 | 做什麼 | 設定 |
|---|---|---|
| 去抖動 | 存檔後等一段安靜期才送出 | `sensai.debounceMs`（預設 1000ms，0 = 關閉） |
| 合併 | 審查中進來的存檔併成**一次**補跑，用最新內容 | 無 |
| 降級 | 補跑那幾輪只做階段一，省一半請求 | 無 |

時間軸（`debounceMs` 為預設 1000ms）：

| 時間 | 事件 | 行為 |
|---|---|---|
| T+0.0s | 存檔 v1 | 開始計時，尚未送出 |
| T+0.4s | 存檔 v2 | 重新計時，仍未送出 |
| T+1.4s | （安靜滿 1s） | 送出 v2 的完整兩階段審查 |
| T+3.0s | 存檔 v3 | 併入待補跑 |
| T+6.0s | v2 結果回來 | 顯示，接著用 v3 補跑**只做階段一** |
| T+8.0s | 補跑結束、沒有新的觸發 | 自動補做完整審查（單一請求） |

四個保證：

1. **持續打字期間幾乎不送出請求。** 只審已經穩定下來的內容。
2. **請求數不隨存檔次數線性增加。**
3. **最後一次存檔的內容一定會被審到。**
4. **降級是延後、不是放棄。** 階段一會濾掉改動範圍外的意見，而 DMA cache、W1C、
   ISR、ABI 這類問題常常不在改動的那幾行上；連續觸發一停止就會自動補完整審查。
   補做時只跑階段二，不重跑階段一 —— 那些意見在降級期間已經送過了。

降級期間側欄會標示「只看剛改動的行 · 連續存檔中，等停下來再做完整審查」。
`sensAI: Review Current File` 不受這三層影響，一律立即執行完整流程。
不同檔案彼此不互相阻擋。

Output → sensAI 可以看到這些決策：

```
[review] 連續存檔中，只審第 40-42 行；完整審查等停下來再做。
[review] uart.c 還在審查中，這次觸發併入下一輪。
[review] 用最新內容補跑 uart.c。
[review] 連續存檔停止，補做 uart.c 的完整審查。
```

### 5.5 CLI 的 exit code（要串 CI 的話讀這段）
...
| 存檔沒有反應，狀態列顯示「沒有改動，未審查」 | **正常行為**：檔案相對 HEAD 沒有改動 | 要重看整份請用 `sensAI: Review Current File`，見 [第 5.3 節](#53-什麼時候才會觸發審查) |
| 存了很多次，只跑了一兩輪審查 | **正常行為**：去抖動 + 合併 | 見 [第 5.4 節](#54-頻繁存檔時的行為) |
| 存檔後隔了約一秒才開始審查 | **正常行為**：`sensai.debounceMs` 的安靜期 | 要立即審查就設 `sensai.debounceMs: 0`，或用 `sensAI: Review Current File` |
| 側欄說「連續存檔中，等停下來再做完整審查」 | **正常行為**：降級成只看改動處 | 停止存檔後會自動補完整審查 |
...
- **不要用 mock 的輸出評估審查品質。** 它是從 prompt 裡挑識別字湊出來的，
  兩則被濾除的意見是刻意植入的測試訊號。
- **exit code 0 不代表沒有問題**，只代表跑完了（見 [第 5.5 節](#55-cli-的-exit-code要串-ci-的話讀這段)）。
  要判斷審查結果請用 `--json`。
